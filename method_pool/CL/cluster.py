...
def parser(argv):
    try:
        opts, args = getopt.getopt(argv, "k:i:o:")
    ...
    for opt, arg in opts:
        if opt == '-i':
            ...
            output_name = arg
    return input_name, k, output_name

if __name__ == "__main__":
    input_name, k, output_name = parser(sys.argv[1:])
    X = load_data(input_name)
    # test_mini_batch_kmeans_manual(X)
    test_mini_batch_kmeans_auto(X, output_name, k)
    # for i in range(4):
    # 23854
    # 99457101
