...
def write_SFC(sfc, cdf, output_filename):
    all_fo = open(output_filename, "w")
    num = len(sfc)
    for i in range(num):
...
def parser(argv):
    ...
    return input_file, output_file, bit_num
...
def run_exp(parameters):
    input_file, output_file, bit_num = parser(parameters)
    source_cdfs = []
    new_cdfs = []
    target_cdfs = []
    labels = []
    ...
    for method_name in method_names:
        source_cdf, new_cdf, target_cdf, min_dist, new_sfc = run(input_file, method_name)
        write_SFC(bit_num, new_sfc, new_cdf, output_file)
